...
//
//   * IT IGNORES THE PERIOD FILTER and carries its own window. The board defaults
//     to Today, and a trend over one day is a dot.
//   * IT PLOTS GATE EVENTS, NOT RAISED PASSES — `verified_at` and
//     `actual_return_date`, never `created_at`. See src/lib/gateActivity.ts for
//     the argument; the short form is that a movement chart drawn on paperwork
//     dates shows traffic on a day the gate was shut.
...
export default function BoardMovementTrend({ rows, loading, activeKey, onSelect }: Props): React.ReactElement {
  ...
  return (
    <BoardCard
      ...
      subtitle={`Cleared out and returned at the gate — last ${days} days, whatever the period filter says.`}
      loading={loading}
      skeletonHeight="h-56"
      control={
